 ...
 * @brief ESP-IDF compatibility layer for the Arduino-shaped MB85RC CLI example.
 *
...
#define MB85RC_EXAMPLE_PLATFORM_IDF 1
#endif
...
class String {
 public:
  ...
  int indexOf(char needle) const {
    const char* found = static_cast<const char*>(std::memchr(_buf, needle, _len));
    if (found == nullptr) {
      return -1;
    }
  ...
  String substring(size_t start, size_t end) const {
    ...
    char tmp[kCapacity] = {};
    const size_t count = (end - start < kCapacity - 1U) ? (end - start) : (kCapacity - 1U);
    std::memcpy(tmp, _buf + start, count);
    tmp[count] = '\0';
    return String(tmp);
  }
...
class TwoWire {
 public:
  ...
  MB85RC::Status writeStatus(uint8_t addr, const uint8_t* data, size_t len,
                             uint32_t timeoutMs) {
    if (data == nullptr || len == 0U) {
      return MB85RC::Status::Error(MB85RC::Err::INVALID_PARAM,
                                   "Invalid I2C write params");
    }
    return mapStatus(transmit(addr, data, len, timeoutMs), "I2C write failed");
  }

  MB85RC::Status writeReadStatus(uint8_t addr, const uint8_t* tx, size_t txLen,
    ...
    if (rxLen == 0U) {
      return mapStatus(transmit(addr, tx, txLen, timeoutMs), "I2C write failed");
    }
    if (txLen == 0U) {
 ...
 private:
  ...
  static MB85RC::Status mapStatus(esp_err_t err, const char* context) {
    if (err == ESP_OK) {
      return MB85RC::Status::Ok();
    }
    if (err == ESP_ERR_TIMEOUT) {
      return MB85RC::Status::Error(MB85RC::Err::I2C_TIMEOUT, context, err);
    }
    if (err == ESP_ERR_INVALID_ARG) {
      return MB85RC::Status::Error(MB85RC::Err::INVALID_PARAM, context, err);
    }
    if (err == ESP_ERR_INVALID_RESPONSE || err == ESP_ERR_NOT_FOUND) {
      return MB85RC::Status::Error(MB85RC::Err::I2C_BUS, context, err);
    }
    return MB85RC::Status::Error(MB85RC::Err::I2C_ERROR, context, err);
  }
